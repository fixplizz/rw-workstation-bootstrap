...
smoke_require_ref() {
  [[ -n ${FIXPLIZZ_SMOKE_REF:-} ]] || smoke_fail 2 'FIXPLIZZ_SMOKE_REF is required (for example v0.1.0-rc7).'
  ...
  [[ $FIXPLIZZ_SMOKE_REF =~ ^v[0-9]+\.[0-9]+\.[0-9]+-rc[0-9]+$ ]] ||
    smoke_fail 2 'FIXPLIZZ_SMOKE_REF must be an immutable RC tag such as v0.1.0-rc7.'
}
